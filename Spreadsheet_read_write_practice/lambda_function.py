...
queries = {'surgery':'''select sd.*,sdl.specialityName,afp.aliasName,cp.customerNumber, date(date_add(dischargeDate,INTERVAL '5:30' HOUR_MINUTE)) as DischargeDate , 
                                    admissionDate as AdmissionDate , pp.patientName , date_add(dischargeDate, INTERVAL '5:30' HOUR_MINUTE) as discharge_date ,
                                    
                                    Month(date_add(dischargeDate, INTERVAL '5:30' HOUR_MINUTE)) as discharge_month , Year(date_add(dischargeDate, INTERVAL '5:30' HOUR_MINUTE)) as discharge_Year ,
...
def lambda_handler(event, context):
    try:
        ...
        # surgery['formatted_date'] = surgery.apply(lambda x: month_year(x['discharge_month'],x['discharge_Year']),axis=1)
        
        #surgery['responce_phase'] = surgery.apply(lambda x: response_chk(x['timelist'],x['responce']),axis=1)
        
        # surgery = surgery[surgery['responce']=='false']
        ...
        Subject = "spreadsheet read write practice ERROR" 
