...
def clear_and_write_to_sheet(sheet_id, sheet_name, sheet_range, data):
    try:
        ...
        #    till_col_fd_temp = 1
        sheet = sheet_name + '!' + sheet_range.format(till_col_fd_temp + 1)  # + str(till_col_fd_temp)
        client_sp.values_clear(sheet)

        # to_add_col = len(data) - till_col_fd_temp
        # if to_add_col < 0:
        #    sheet_obj.add_rows(rows=abs(to_add_col))
        sheet_obj.resize(rows=len(data) + 1)

        print(till_col_fd_temp, len(data))
        range_str = sheet_range.format(len(data) + 1)  # + str(len(data))
