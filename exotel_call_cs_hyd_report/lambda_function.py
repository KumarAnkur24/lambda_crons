...
today = time.date()
print(today)
d1_ = time - timedelta(minutes=30)
d1 = d1_.strftime("%H:%M:%S")
print(d1)
d2_ = time - timedelta(minutes=20)
...
def lambda_handler(event, context):
    try:
        ...
        if len(data):
            ...
            email_recipient_list = ['nikunj.r@ayu.health', 'nikeela@ayu.health', 'rohithm@ayu.health',
                                    'neha@ayu.health', 'shashank@ayu.health', 'vijay.g@ayu.health', 'tls@ayu.health',
                                    'chaitanya.s@ayu.health']
            send_email(None, email_recipient_list, Subject.format(today=today), 'Hi All', res, [fpath])



    except Exception as e:
        Subject = 'Inbound calling detailed view Error | HYD | {today}'
        email_recipient_list = ['nikunj.r@ayu.health']
        send_email(None, email_recipient_list, Subject.format(today=today), 'Hi All', None, [])
        raise e